getallTapparticipants<- function(experiment) {
  ...
  outputfilename<- sprintf('time_model%d_active reach locations_they went_there_Prop.csv', experiment)
...
getanglesforTaptask<- function (filename) {
  ...
  Tapdeviations<- (((atan2(df$handx_cm, df$handy_cm) / pi) * 180) +90) - df$targetangle_deg
