...
fitAsymptoticDecayModels <- function() {
  ...
  for (group in names(groupsignals)) {
    ...
    for (signalname in groupsignals[[group]]) {
      ...
      # set up variables for output data frame:
      participant <- c()
      lambda <- c()
      N0 <- c()
      set <- c()
      ...
      for (pp in participants) {
        ...
        for (trialset in c('main','reversal')) {
          ...
          cat(sprintf('group: %s, signal: %s, participant: %s, set: %s\n', group, signalname, pp, trialset))
          
          BL <- baselines[[group]][[signalname]]
          
          indices <- trialsets[[trialset]] + BL
          signal <- df[,pp][ indices ]
          
          # we need to baseline for reversal modeling:
          if (trialset == 'reversal') {
            if (signalname == 'slowprocess') {
              signal <- signal - df[,pp][ min(indices) - 1 ]
            } else {
              a_i <- c(81:160) + BL
              signal <- signal - mean(df[,pp][a_i], na.rm=TRUE)
            }
          }
          
          schedule <- schedules[[group]][[signalname]]
          signal <- signal * schedule
          if (trialset == 'reversal') {
            signal <- signal *-1
          }
          
          schedule <- rep(-1, length(signal))
      ...
      fits <- data.frame(participant, set, lambda, N0)

      write.csv(fits, sprintf('data/%s_%s_rate.csv',group, signalname), row.names = F, quote = F)
